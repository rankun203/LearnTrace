...
import com.mindfine.hibernate.orMapping.VO.SuperTest;
...
public class HibernateQLTest extends SuperTest{
    private static SessionFactory sf;

    public HibernateQLTest (){
        this.sf = SessionFactoryTool.getSessionFactory();
    }
...
//		q.setParameter(1, 8);
    ...
    public void testHQL_12() {
        Session session = sf.openSession();
        session.beginTransaction();
        Query q = session.createQuery("select new com.mindfine.hibernate.orMapping.VO.QL.MsgInfo (m.id, m.cont, m.topic.title, m.topic.category.name) from Msg m");
